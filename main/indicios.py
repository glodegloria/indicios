...
#Function to run the Metropolis-Hastings algorithm for each chain
# It takes the chain index as input and returns the output of the inditek_metropolis function
def run_chain(iChain):
    params_current = np.transpose(initial_theta[iChain,:])
              
    output=inditek_metropolis(LonDeg, landShelfOcean_Lat,landShelfOcean_Lon, landShelfOceanMask, d_obis,se_obis,idx_obis, shelf_lonlatAge, Point_timeslices, food_shelf, temp_shelf, params_current, mu, sigma, sigma_prop, ran,  nsamples, nparams)
...
#To measure the time taken for the entire process
start=time.time()

#MH-MCMC SETUP:
num_chains=1 #number of chains to run in parallel (2 for now, but we can increase it)
nsamples=7 # number of samples does is done in each chain, number of times that the M-H algorithm is run in each chain (3 for now, but we can increase it)
nparams=5 #number of parameters to be estimated (5 for now, but we can increase it) 
...
mu=np.array([200,0.15,2,0.01,0.0005])
...
sigma=np.array([80,0.05,0.2,0.005,0.00025])

#which parameters to consider with gaussian distribution instead of uniform distribution (no a priori, only bounds: range)

gaus=np.array([3]) # for now we only believe that Q10 should fall around the value 2 
...
ran=np.array([[100,400],[0.01,0.3],[1,3],[0,0.2],[-0.001,0.001]]) #range for parameter volume sampling to start the chains from equidistant points in the hipervolume
initial_theta=latin_hypercube_sampling_ichains(ran[:,0], ran[:,1], num_chains)

c=0.35 #for tunning independently (implement an adaptive search-window along the burn-in iterations to approach 0.25 acceptance frequency for all parameters)
sigma_prop=c*sigma
...
ran=np.array([[50,500],[0.001,np.inf],[1,3],[0,np.inf],[-np.inf,np.inf]])
...
#Calls the Metropolis function for each chain in parallel using joblib
# The function run_chain is defined above
...
#For each chain, we store the results in the pre-allocated arrays
