...
for c in range(6):
    #for a in range(D_shelf.shape[0]):
        for t in range(50,shelf_lonlatAge.shape[1]):
            ...
            ind_pac=ind_pac[~np.isnan(ind_pac)].astype(int)
            D_pac=D_shelf[:,ind_pac,t,c]
            for a in range(D_shelf.shape[0]):
                A[:len(ind_pac), t] = D_pac[a,:]
            ...
            ax1.plot(-year[~np.isnan(year)], mean_values[~np.isnan(year)], color='darkblue', alpha=0.8, linewidth=0.5, label='Media')
...
for t in range(len(time_point)):
    ...
    ax.plot(landShelfOcean_Lon, min_lat[t,:], color='orange', linestyle='--', label='Isoline 25ºC', transform=ccrs.PlateCarree())
    ax.plot(landShelfOcean_Lon, max_lat[t,:], color='orange', linestyle="--", transform=ccrs.PlateCarree())
...
ax.legend(loc='upper left', bbox_to_anchor=(-2.1, 1.3), fontsize=8)
